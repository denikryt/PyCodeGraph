...
function loadGraph(data,label='artifact'){
 ...
 state.selectedId=null;state.selectedModuleId=null;buildFilters();buildOutline();selectProjection('architecture');updateSelectionPanel();
}
...
function buildOutline(){if(!state.graph)return;els.outline.innerHTML='';for(const m of state.graph.entities.filter(e=>e.kind==='module').sort((a,b)=>(a.qualified_name||'').localeCompare(b.qualified_name||''))){const b=document.createElement('button');b.className='outline-module';b.textContent=m.qualified_name||m.name;b.dataset.id=m.id;b.onclick=()=>{selectNode(m.id);state.selectedModuleId=m.id;state.scope='module';els.scopeSelect.value='module';selectProjection('structure');};els.outline.appendChild(b);}refreshOutlineSelection();}
function refreshOutlineSelection(){els.outline.querySelectorAll('.outline-module').forEach(b=>b.classList.toggle('selected',b.dataset.id===state.selectedModuleId));}
function selectProjection(name){state.projection=name;els.projectionButtons.querySelectorAll('button').forEach(b=>b.classList.toggle('active',b.dataset.projection===name));if(name==='architecture'){state.scope='overview';els.scopeSelect.value='overview';}if(name==='neighborhood'&&!state.selectedId){const e=state.graph?.entities.find(x=>x.kind==='module')||state.graph?.entities[0];if(e)state.selectedId=e.id;}rebuildProjection();}
...
function buildProjection(k){if(k==='architecture')return architectureProjection();if(k==='structure')return structureProjection();if(k==='types')return relationProjection(new Set(['class','type','external']),new Set(['inherits','uses_type']));if(k==='calls')return relationProjection(new Set(['class','function','method','external']),new Set(['calls']));if(k==='neighborhood')return neighborhoodProjection();return customProjection();}
...
function structureProjection(){let mid=state.selectedModuleId;if(!mid&&state.selectedId)mid=state.moduleOf.get(state.selectedId);if(!mid)mid=state.graph.entities.find(e=>e.kind==='module')?.id;if(!mid)return{nodes:[],edges:[]};state.selectedModuleId=mid;refreshOutlineSelection();const ids=new Set([mid]),q=[{id:mid,d:0}],max=Math.max(1,state.depth+1);while(q.length&&ids.size<state.limit){const {id,d}=q.shift();if(d>=max)continue;for(const c of state.children.get(id)||[]){const e=state.entityById.get(c);if(!e||!state.entityKinds.has(e.kind)||['parameter','variable'].includes(e.kind))continue;if(!ids.has(c)){ids.add(c);q.push({id:c,d:d+1});if(ids.size>=state.limit)break;}}}const edges=state.graph.relations.filter(r=>r.kind!=='contains'&&state.relationKinds.has(r.kind)&&ids.has(r.source)&&ids.has(r.target)&&!(r.kind==='reads'&&r.attributes?.role==='parameter'));return{nodes:[...ids].map(id=>state.entityById.get(id)).filter(Boolean),edges};}
...
function neighborhoodProjection(kindOverride=null,relOverride=null){if(!state.selectedId)return{nodes:[],edges:[]};const kinds=kindOverride||state.entityKinds,rels=relOverride||state.relationKinds,ids=new Set([state.selectedId]);let front=new Set([state.selectedId]);for(let d=0;d<state.depth&&ids.size<state.limit;d++){const next=new Set();for(const id of front){for(const r of [...(state.outgoing.get(id)||[]),...(state.incoming.get(id)||[])]){if(!rels.has(r.kind))continue;const other=r.source===id?r.target:r.source,e=state.entityById.get(other);if(!e||!kinds.has(e.kind))continue;if(!ids.has(other)){ids.add(other);next.add(other);if(ids.size>=state.limit)break;}}}front=next;if(!front.size)break;}const edges=state.graph.relations.filter(r=>rels.has(r.kind)&&ids.has(r.source)&&ids.has(r.target));return{nodes:[...ids].map(id=>state.entityById.get(id)).filter(Boolean),edges};}
...
function makeVisualNode(e){return{...e,x:0,y:0,w:240,h:80,inputPorts:[],outputPorts:[],relationGroups:[],rows:[],portMap:new Map(),_manual:false};}
...
function selectNode(id){if(!id||!state.entityById.has(id))return;state.selectedId=id;const m=state.moduleOf.get(id);if(m)state.selectedModuleId=m;updateSelectionPanel();refreshOutlineSelection();draw();}
...
els.fileInput.onchange=e=>openFile(e.target.files?.[0]);els.projectionButtons.onclick=e=>{const b=e.target.closest('button[data-projection]');if(b&&state.graph)selectProjection(b.dataset.projection);};els.searchInput.oninput=e=>performSearch(e.target.value);els.depthInput.oninput=e=>{state.depth=Number(e.target.value);els.depthOutput.value=state.depth;rebuildProjection();};els.limitInput.value=state.limit;els.limitInput.onchange=e=>{state.limit=Math.max(20,Math.min(800,Number(e.target.value)||180));e.target.value=state.limit;rebuildProjection();};els.scopeSelect.onchange=e=>{state.scope=e.target.value;rebuildProjection();};els.selectAllFilters.onclick=()=>{state.entityKinds=new Set(ENTITY_KINDS);state.relationKinds=new Set(RELATION_KINDS);buildFilters();rebuildProjection();};els.focusBtn.onclick=()=>{state.scope='selection';els.scopeSelect.value='selection';selectProjection('neighborhood');};els.moduleBtn.onclick=()=>{if(!state.selectedId)return;state.selectedModuleId=state.moduleOf.get(state.selectedId);if(state.selectedModuleId){state.scope='module';els.scopeSelect.value='module';selectProjection('structure');}};els.fitBtn.onclick=fitToView;els.resetBtn.onclick=()=>{state.expandedGroups.clear();state.selectedId=null;state.selectedModuleId=null;state.scope='overview';els.scopeSelect.value='overview';selectProjection('architecture');updateSelectionPanel();};
