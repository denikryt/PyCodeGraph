...
function loadGraph(data,label='artifact'){
 ...
 state.selectedId=null;state.selectedModuleId=null;state.navigationHistory=[];buildFilters();buildOutline();selectProjection('architecture');updateSelectionPanel();
}
...
function navigationSnapshot(){return{projection:state.projection,scope:state.scope,selectedId:state.selectedId,selectedModuleId:state.selectedModuleId,camera:{...state.camera}};}
function pushNavigationHistory(){
 ...
 if(prev&&prev.projection===snap.projection&&prev.scope===snap.scope&&prev.selectedId===snap.selectedId&&prev.selectedModuleId===snap.selectedModuleId)return;
...
function restoreNavigationSnapshot(snap){
 ...
 state.projection=snap.projection;state.scope=snap.scope;state.selectedId=snap.selectedId;state.selectedModuleId=snap.selectedModuleId;
 ...
 updateSelectionPanel();refreshOutlineSelection();updateNavigationUI();draw();
...
function updateNavigationUI(){
 ...
 const labels={architecture:'Architecture',structure:'Structure',types:'Types',calls:'Call graph',neighborhood:'Neighborhood',custom:'Custom'};
 let parts=[];
 ...
 else if(state.projection==='neighborhood'&&state.selectedId){parts=entityPath(state.selectedId).map(e=>e.name||e.qualified_name||e.id);}
...
function neighborhoodProjection(kindOverride=null,relOverride=null){if(!state.selectedId)return{nodes:[],edges:[]};const kinds=kindOverride||state.entityKinds,rels=relOverride||state.relationKinds,hiddenStructural=new Set(['contains','accepts','returns']),ids=new Set([state.selectedId]);let front=new Set([state.selectedId]);for(let d=0;d<state.depth&&ids.size<state.limit;d++){const next=new Set();for(const id of front){for(const r of [...(state.outgoing.get(id)||[]),...(state.incoming.get(id)||[])]){if(!rels.has(r.kind)||hiddenStructural.has(r.kind))continue;const other=r.source===id?r.target:r.source,e=state.entityById.get(other);if(!e||!kinds.has(e.kind))continue;if(!ids.has(other)){ids.add(other);next.add(other);if(ids.size>=state.limit)break;}}}front=next;if(!front.size)break;}const edges=state.graph.relations.filter(r=>rels.has(r.kind)&&!hiddenStructural.has(r.kind)&&ids.has(r.source)&&ids.has(r.target));return{nodes:[...ids].map(id=>state.entityById.get(id)).filter(Boolean),edges};}
...
function selectNode(id){if(!id||!state.entityById.has(id))return;state.selectedId=id;const m=state.moduleOf.get(id);if(m)state.selectedModuleId=m;updateSelectionPanel();refreshOutlineSelection();updateNavigationUI();draw();}
